...
struct v_t {
    ...
    v_t operator+(const v_t&v)const {
        return v_t(x+v.x,y+v.y,z+v.z);
    }
    v_t operator-(const v_t&v)const {
        return v_t(x-v.x,y-v.y,z-v.z);
    }
    ...
    v_t operator*(const double d)const {
        return v_t(x*d,y*d,z*d);
    }
    v_t cross(const v_t&v)const {
        return v_t(y*v.z-z*v.y,z*v.x-x*v.z,x*v.y-y*v.x);
    }
    v_t norm()const {
        return*this*(1./sqrt(magsqr()));
    }
    double dot(const v_t&v)const {
...
struct sphere_t {
    ...
    v_t get_normal(const v_t&v)const {
        return(v-o)*(1./r);
    }
...
struct node_t { /*a bvh in array form+skip for navigation.*/
    ...
    node_t() {}
    node_t(const sphere_t&b,const sphere_t&l,const long jump) :bound(b),leaf(l),diff(jump) {}
    template<bool shadow>
    static void intersect(const ray_t &ray,hit_t &hit) {
        const node_t*p=pool;
        while(p < end) {
...
static node_t *create(node_t*n,const int lvl,int dist,v_t c,v_t d,double r) {
    ...
    const basis_t b(d);
    const double nr=r*1/3.,daL=2.*M_PI/6.,daU=2.*M_PI/3.;
    double a=0;
